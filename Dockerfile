...
# 작업 영역 설정 (이제부터 수행하는 작업은 /myapp 디렉터리에서 수행)
WORKDIR /myapp
...
COPY --from=mvn-build /myapp/target/*.war /usr/local/tomcat/webapps/deploy.war

# 첫 번째 스테이지를 제거하고, Github runner의 war 파일을 복사하는 경우에 사용하는 COPY
# Github runner의 파일을 COPY하는 경우 Github runner의 작업 디렉터리를 기준으로 실행
